...
def nn_forward_pass(params, X):
    ...
    hidden = torch.max(Z1, torch.zeros_like(Z1))
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    max_scores, _ = torch.max(scores, dim = 1, keepdims=True)
    scores -= max_scores
    exp_scores = torch.exp(scores)
    sum_exp_scores = exp_scores / torch.sum(exp_scores, dim = 1, keepdims=True)

    loss = sum_exp_scores[torch.arange(num_train), y]
    loss = -torch.sum(torch.log(loss)) / num_train
    ...
    sum_exp_scores[torch.arange(num_train), y] -= 1
    sum_exp_scores /= num_train

    dW1 = torch.matmul(sum_exp_scores, W2.T) #(N,H)
    Z1 = torch.matmul(X, W1) + b1
    dZ1 = dW1 * (Z1 > 0) #(N,H)
    dW1 = torch.matmul(X.T, dZ1)

    hidden = torch.max(Z1, torch.zeros_like(Z1))
    dW2 = torch.matmul(hidden.T, sum_exp_scores) #(H,C)

    db1 = torch.sum(dZ1, dim = 0) #(D,H)
    grads['b1'] = db1
    
    db2 = torch.sum(sum_exp_scores, dim = 0)
    grads['b2'] = db2
...
def nn_predict(params, loss_func, X):
  ...
  dot_mat_1 = torch.matmul(X, params['W1']) + params['b1']
  max_val = torch.max(dot_mat_1, torch.zeros_like(dot_mat_1))
  dot_mat_2 = torch.matmul(max_val, params['W2']) + params['b2']
  y_pred = torch.argmax(dot_mat_2, dim = 1)
...
def nn_get_search_params():
  ...
  learning_rates = [1e0, 1e1]
  hidden_sizes = [2, 256]
  regularization_strengths = [1e-8, 1e-12]
    ...
    for hs in hidden_sizes:
      for reg in regularization_strengths:
        for lrd in learning_rate_decays:
          ...
          stat = net.train(data_dict['X_train'], data_dict['y_train'],
                                    data_dict['X_val'], data_dict['y_val'],
                                    num_iters=500, batch_size=1000,
                                    learning_rate=lrs, learning_rate_decay=lrd,
                                    reg=reg, verbose=False)
          
          y_val_pred = net.predict(data_dict['X_val'])
          acc_val = 100.0 * (y_val_pred == data_dict['y_val']).double().mean().item()
          # print(lrs, hs, reg, lrd)
          # print('Val acc: %.2f%%' % acc_val)
          if acc_val > best_acc:
            # print('start------------')
            # print(lrs, hs, reg, lrd)
            # print(acc_val)
            ...
            best_dict['reg'] = reg
            best_dict['lrd'] = lrd
            best_dict['acc'] = acc_val
            best_dict['stat'] = stat
            best_dict['net'] = net
            best_dict['lrs'] = lrs
  
  best_net = best_dict['net']
  best_stat = best_dict['stat']
  best_val_acc = best_dict['acc']
