  ...
  for i in range(num_train):
    ...
    for j in range(num_classes):
      ...
      if margin > 0:
        ...
        dW[:, j] += X[i, :] #incorrect class grad
        dW[:, y[i]] -= X[i, :]	#correct class grad
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  # find scores, correct scores and loss
  scores = torch.matmul(X, W) #(N,C)
  correct_scores = scores[torch.arange(num_train), y] #(N,)
  loss_margins = scores - correct_scores.reshape(correct_scores.shape[0], 1) + 1 #(N,1), delta = 1
  loss_margins[torch.arange(num_train), y] = 0 #ensure correct scores dont go in loss

  # loss with regularization
  loss = torch.sum(loss_margins) / num_train
  loss += reg * torch.sum(W * W)
  ...
  # find positive loss margins, gradient
  pos_margins = loss_margins.clone()
  pos_margins[pos_margins > 0] = 1
  pos_count = torch.sum(pos_margins, dim = 1)
  pos_margins[torch.arange(num_train), y] -= pos_count #minus correct class
  dW = torch.matmul(X.T, pos_margins) / num_train #(D,N) x (N,C) = (D,C)
...
def sample_batch(X, y, num_train, batch_size):
  ...
  # reshape data
  # X_batch = torch.reshape(X_batch, (X_batch.shape[0],  X_batch.shape[2]))
...
def predict_linear_classifier(W, X):
  ...
  y_pred = torch.argmax(torch.matmul(X, W), dim=1) #(N,)
...
def svm_get_search_params():
  ...
  # default values work pretty well
  learning_rates = [1e-2, 1e0, 1e2, 1e4]
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  # train classifier (using code from given cells (.ipynb file) as reference)
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
      ...
      # shift values for numeric stability (no overflow)
      max_score, _ = torch.max(scores, dim = 0)
      scores -= max_score
      sum_exp_scores = torch.sum(torch.exp(scores))
      correct_class_score_exp = torch.exp(scores[y[i]])
      loss -= torch.log(correct_class_score_exp / sum_exp_scores)

      # correct class grad
      dW[:, y[i]] -= (sum_exp_scores - correct_class_score_exp) / sum_exp_scores * X[i]

      for j in range(num_classes):
        if j != y[i]:
          # incorrect class grad
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  scores -= max_scores.reshape(max_scores.shape[0], 1)
  exp_scores = torch.exp(scores)
  sum_exp_scores = torch.sum(exp_scores, dim=1, keepdim=True)
  mat = torch.exp(scores)/sum_exp_scores

  # softmax loss
  loss = mat[torch.arange(num_train), y]
  loss = torch.sum(-torch.log(loss))
  loss /= num_train
  loss += reg * torch.sum(W * W)

  # softmax gradient
  mat[torch.arange(num_train), y] -= 1
