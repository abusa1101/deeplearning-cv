...
class ProposalModule(nn.Module):
  def __init__(self, in_dim, hidden_dim=256, num_anchors=9, drop_ratio=0.3):
    ...
    A = self.num_anchors
    self.pred_layer = nn.Sequential(nn.Conv2d(in_dim, hidden_dim, 3, padding=1),
                                    nn.Dropout(drop_ratio),
                                    nn.LeakyReLU(),
                                    nn.Conv2d(hidden_dim, 6 * A, 1))
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    features = features.reshape(B, self.num_anchors, 6, H, W)

    if mode == 'train': # Train Mode
      # Extract info 
      offsets = self._extract_anchor_data(features[:,:,2:,:,:], pos_anchor_idx)
      conf_pos = self._extract_anchor_data(features[:,:,:2,:,:], pos_anchor_idx)
      conf_neg = self._extract_anchor_data(features[:,:,:2,:,:], neg_anchor_idx)

      # Get scores
      conf_scores = torch.cat((conf_pos, conf_neg), dim=0)
      conf_scores = conf_scores[:, 0:2]

      # Compute proposal coords
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    # Image feature extraction, Grid and anchor generation
    features = self.feat_extractor(images)
    B = images.shape[0]
    grid = GenerateGrid(B)
    anc_list = GenerateAnchor(self.anchor_list.to(grid.device, grid.dtype), grid)

    # Compute IoU between anchors and GT boxes, and determine activated/negative 
    # anchors, GT_conf_scores (?), GT_offsets, GT_class
    iou_data = IoU(anc_list, bboxes)
    temp1, temp2, _, temp3, temp4, temp5,_ = ReferenceOnActivatedAnchors(anc_list, 
                                                                         bboxes, 
                                                                         grid, 
                                                                         iou_data,
                                                                         method='FasterRCNN')
    pos_anchor_idx = temp1 # assigning temps just to keep things visually clean
    neg_anchor_idx = temp2
    GT_offsets = temp3
    GT_class = temp4
    activated_anchor_coords = temp5

    anc_per_img = torch.prod(torch.tensor(anc_list.shape[1:-1]))

    # Compute conf_scores, offsets, proposals
    conf_scores, offsets, proposals = self.prop_module(features,
                                                       activated_anchor_coords, 
                                                       pos_anchor_idx, 
                                                       neg_anchor_idx)
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
      ...
      # Compute conf_scores, proposals, class_prob 
      ...
      for i in range(B): #for image i
        cs_i = conf_scores[i]
        props_i = proposals[i]
        
        # conf scores and nms filtering
        cs_mask = cs_i > thresh
        cs_i = cs_i[cs_mask]
        props_i = props_i[cs_mask, :] #A*H*W * 4
        nms_mask = torchvision.ops.nms(props_i, cs_i, nms_thresh)

        final_conf_probs.append(cs_i[nms_mask].unsqueeze(1))
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    rpn_loss, conf_scores , proposals, features, GT_class, pos_anchor_idx, anc_per_img = \
      self.rpn.forward(images, bboxes, 'all')

    # ROI Align and meanpool
    batch_index = (pos_anchor_idx // anc_per_img).unsqueeze(1).to(proposals.dtype) # 0-9
    proposals_w_batch_index = torch.cat((batch_index, proposals),dim=1)
    aligned_features = torchvision.ops.roi_align(features, 
                                                 proposals_w_batch_index, 
                                                 (self.roi_output_w, 
                                                  self.roi_output_h)) 
    pooled_features = torch.mean(aligned_features,(2, 3))

    # Get predictions: Pass RoI feature through cls_layer
    scores = self.cls_layer(pooled_features)
    
    # Compute cross entropy loss
    cls_loss = nn.functional.cross_entropy(scores, GT_class, reduction='sum') * 1. / B
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    # Compute final_proposals and final_conf_probs
    ...
    # Compute final_class
    aligned_feat = torchvision.ops.roi_align(features, 
                                             final_proposals, 
                                             (self.roi_output_w, 
                                              self.roi_output_h))
    pooled_feat = torch.mean(aligned_feat, (2, 3))
    scores = self.cls_layer(pooled_feat)
    cls = torch.max(scores, 1)[1].to(scores.dtype).unsqueeze(1)

    final_class = []
    count = 0
    for i, val in enumerate(final_proposals):
      final_class.append(cls[count:count + len(val)])
      count += len(val)
