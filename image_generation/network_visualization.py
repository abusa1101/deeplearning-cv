...
def compute_saliency_maps(X, y, model):
  ...
  saliency = torch.max(saliency, axis=1)[0]
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(100):
    scores = model(X_adv) 
    ypred = torch.max(scores, 1)[1]

    if (ypred == target_y):
      print("Iteration %d: target score %.3f, max score %.3f" % (i, target_y, 
                                                                 ypred))
      break
    ...
    X_adv.grad.data.zero_() #zeroing grad
    print("Iteration %d: target score %.3f, max score %.3f" % (i, target_y, 
                                                               ypred))
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    objective = scores[:, target_y] - (l2_reg * torch.norm(img)**2)
    objective.backward()

    # learning_rate * (regularization gradient + image gradient)
    img_grad = learning_rate * (img.grad.data) # dont div by norm(img.grad.data)
    img.data += img_grad.data
    img.grad.data.zero_()
