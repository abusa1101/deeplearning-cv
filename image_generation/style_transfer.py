...
def content_loss(content_weight, content_current, content_original):
    ...
    content_sum = torch.sum(torch.square(content_current - content_original))
...
def gram_matrix(features, normalize=True):
    ...
    reshaped = features.view(N, C, -1)
    gram = torch.matmul(reshaped, reshaped.transpose(1, 2))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    style_loss = 0
    for i,_ in enumerate(style_layers):
        gram = gram_matrix(feats[style_layers[i]])
        sum_grams = torch.sum(torch.square(gram - style_targets[i]))
        style_loss += style_weights[i] * sum_grams
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    var1 = torch.sum(torch.square(img[:, :, :, :-1] - img[:, :, :, 1:]))
    var2 = torch.sum(torch.square(img[:, :, :-1, :] - img[:, :, 1:, :]))
    tv_loss = tv_weight * (var1 + var2)
    return tv_loss
