...
def three_layer_convnet(x, params):
  ...
  l1_output = F.conv2d(x, conv_w1, conv_b1, padding=2)
  l2_output = F.relu(l1_output)
  l3_output = F.conv2d(l2_output, conv_w2, conv_b2, padding=1)
  l4_output = F.relu(l3_output)
  scores = F.linear(flatten(l4_output), fc_w, fc_b)
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  conv_w1 = nn.init.kaiming_normal_(torch.empty(channel_1, C, 
                                                kernel_size_1, 
                                                kernel_size_1, 
  ...
  conv_w2 = nn.init.kaiming_normal_(torch.empty(channel_2, 
                                                channel_1, 
                                                kernel_size_2, 
                                                kernel_size_2, 
                                                dtype=dtype, 
  ...
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, 
                                             channel_2 * H * W, 
                                             dtype=dtype, 
                                             device=device))
...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    self.conv1 = nn.Conv2d(in_channel, channel_1, filter1, padding=2)
    nn.init.kaiming_normal_(self.conv1.weight)

    self.conv2 = nn.Conv2d(channel_1, channel_2, filter2, padding=1)
...
def initialize_three_layer_conv_part4():
  ...
  model = nn.Sequential(nn.Conv2d(C, channel_1, kernel_size_1, 
                                  padding=pad_size_1),
                        nn.ReLU(),
                        nn.Conv2d(channel_1, channel_2, kernel_size_2, 
                                  padding=pad_size_2),
                        nn.ReLU(),
                        Flatten(),
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if downsample:
      stride = 2
      self.shortcut = nn.Sequential(nn.Conv2d(Cin, Cout, 1, stride))
    else:
      stride = 1
      if Cin == Cout:
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    res_blocks = [ResNetStem(Cin=Cin, Cout=stage_args[0][0])]
    for stage_arg in stage_args:
      res_blocks.append(ResNetStage(*stage_arg, block=block)) 
      # Note: '*var' is shortcut for 'var[0],var[1],...var[n]' (from A2 I think) 
      #       i.e. it collects all positional arguments in the tuple
    self.cnn = nn.Sequential(*res_blocks)
  ...
  def forward(self, x):
    ...
    scores = self.cnn(x)
    pool = nn.AvgPool2d(scores.shape[2:])
    scores = pool(scores)
