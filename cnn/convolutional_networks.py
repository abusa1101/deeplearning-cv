...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    x_padded = torch.nn.functional.pad(x, (pad, pad, pad,pad), mode = 'constant')
      ...
      for f in range(F):
        for h in range(H_prime):
          for k in range(W_prime):
            HS = h * stride
            KS = k * stride
            x_new = x_padded[n, :, HS:(HS + HH), KS:(KS + WW)]
            out[n, f, h, k] = torch.sum(x_new * w[f]) + b[f]
      ...
      for f in range(F):
        for h in range(H_prime):
          for k in range(W_prime):
            ...
            KS = k * stride
            x_new = x_padded[n, :, HS:(HS + HH), KS:(KS + WW)]
            dw[f] += x_new * dout[n, f, h, k]
            dx[n, :, HS:(HS + HH), KS:(KS + WW)] += w[f] * dout[n, f, h, k]
            db[f] += torch.sum(dout[n, f, h, k], dim=0, keepdims=True)
      ...
      for c in range(C):
        for h in range(H_prime):
          for k in range(W_prime):
            HS = h * stride 
            KS = k * stride
            out[n, c, h, k] = torch.max(x[n, c, HS:(HS + pool_h), KS:(KS + pool_w)])
      ...
      for c in range(C):
        for h in range(H_prime):
          for k in range(W_prime):
            HS = h * stride 
            KS = k * stride 
            x_new = x[n, c, HS:(HS + pool_h), KS:(KS + pool_w)]
            max_val = torch.max(x_new)
            dx[n, c, HS:(HS + pool_h), KS:(KS + pool_w)] += (x_new == max_val) * \
                                                            dout[n, c, h, k]
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    W2 = weight_scale * torch.randn(int(num_filters * H/2 * W/2), hidden_dim, 
                                    dtype=dtype, device=device)
    b2 = torch.zeros(hidden_dim, dtype=dtype, device=device)
    self.params['W2'] = W2
    self.params['b2'] = b2

    W3 = weight_scale * torch.randn(hidden_dim, num_classes, dtype=dtype, 
                                    device=device)
  ...
  def loss(self, X, y=None):
    ...
    dx3, dW3, db3 = Linear.backward(dscores, cache3)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    #exp. reduction in scale for each pooling iteration
    dims =  int(H / 2**len(max_pools)) * int(W / 2**len(max_pools)) * num_filters[-1]
    last_layer = str(len(num_filters) + 1)

    if weight_scale is 'kaiming':
      # If kaiming: Initialize first layer
      ...
      # If kaiming: Initiaze middle layers
      for i in range(1, len(num_filters)):
        layer = str(i + 1)
        self.params['W' + layer] = kaiming_initializer(num_filters[i - 1], 
                                                       num_filters[i], K=3,
                                                       relu=True, dtype=dtype, 
                                                       device=device)
        self.params['b' + layer] = torch.zeros(num_filters[i], dtype=dtype, device=device)
        if self.batchnorm:
          self.params['gamma' + layer] = torch.ones(num_filters[i], dtype=dtype, device=device)
          self.params['beta' + layer] = torch.zeros(num_filters[i], dtype=dtype, device=device)
      
      # If kaiming: Initiaze last layer
      self.params['W' + last_layer] = kaiming_initializer(dims, num_classes, 
                                                          dtype=dtype, 
                                                          device=device)
      self.params['b' + last_layer] = torch.zeros(num_classes,dtype=dtype, 
                                                  device=device)

    else:
      # If not kaiming: Initialize first layer
      self.params['W1'] = weight_scale * torch.randn(num_filters[0], C, HH, WW, 
                                                     dtype=dtype, device=device)
      self.params['b1'] = torch.zeros(num_filters[0], dtype=dtype, device=device)
      if self.batchnorm:
        self.params['gamma1'] = torch.ones(num_filters[0], dtype=dtype, device=device)
      ...
      # If not kaiming: Initialize middle layers
      for i in range(1, len(num_filters)):
        layer = str(i + 1)
        self.params['W' + layer] = weight_scale *  torch.randn(num_filters[i], 
                                                               num_filters[i - 1], 
                                                               HH, WW, dtype=dtype, 
                                                               device=device)
        self.params['b' + layer] = torch.zeros(num_filters[i], dtype=dtype, device=device)
      ...
      # If not kaiming: Initialize last layer
      self.params['W' + last_layer] = weight_scale * torch.randn(dims,
                                                                 num_classes,
                                                                 dtype=dtype, 
                                                                 device=device)
      self.params['b' + last_layer] = torch.zeros(num_classes, dtype=dtype, 
  ...
  def load(self, path, dtype, device):
    ...
    for p in self.bn_params:
      self.bn_params[p] = self.bn_params[p].type(dtype).to(device)
    print("load checkpoint file: {}".format(path))
  ...
  def loss(self, X, y=None):
    ...
    # arch: {conv - [batchnorm?] - relu - [pool?]} x (L - 1) - linear
    scores = X
    cache_dict = {}
    ...
    for i in range(self.num_layers - 1):
      ...
      cache_i = 'cache' + str(i + 1)

      if self.batchnorm:
        gamma = self.params['gamma' + str(i + 1)]
        beta = self.params['beta' + str(i + 1)]
        if i in self.max_pools:
          scores, cache = Conv_BatchNorm_ReLU_Pool.forward(scores, W_i, b_i, 
                                                           gamma, beta, 
                                                           conv_param, 
                                                           self.bn_params[i], 
                                                           pool_param)
          cache_dict[cache_i] = cache
        else:
          scores, cache = Conv_BatchNorm_ReLU.forward(scores, W_i, b_i, gamma, 
                                                      beta, conv_param,
                                                      self.bn_params[i])
          cache_dict[cache_i] = cache
      else:
        if i in self.max_pools:
          scores, cache = Conv_ReLU_Pool.forward(scores, W_i, b_i, conv_param,
                                                 pool_param)
          cache_dict[cache_i] = cache
        else:
          scores, cache = Conv_ReLU.forward(scores, W_i, b_i, conv_param)
          cache_dict[cache_i] = cache

    # Linear Layer
    lin_layer = scores.contiguous().view([X.shape[0], -1])
    last_layer = str(self.num_layers)
    W_last = self.params['W' + last_layer]
    b_last = self.params['b' + last_layer]
    scores, cache_dict['cache' + last_layer] = Linear.forward(lin_layer, W_last, b_last)
    ...
    # Linear Layer
    dx, dw, db = Linear.backward(dscores, cache_dict['cache' + last_layer])
    lin_dscores = dx
    grads['W' + last_layer] = dw
    grads['b' + last_layer] = db

    temp_key = str(self.num_layers - 1)
    dscores = lin_dscores.view([N, self.params['W' + temp_key].shape[0], \
                                int(H / 2**len(self.max_pools)), \
                                int(W / 2**len(self.max_pools))])
    ...
    for i in range(self.num_layers - 2, -1, -1):
      ...
      if self.batchnorm:
        ...
        beta_i = 'beta' + str(i + 1)
        if i in self.max_pools:
          dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dscores, cache_dict[cache_i])
          dscores = dx
          grads[W_i] = dw
          grads[b_i] = db
          ...
          dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU.backward(dscores, cache_dict[cache_i])
          dscores = dx
          grads[W_i] = dw
          grads[b_i] = db
          grads[gamma_i] = dgamma
        ...
        if i in self.max_pools:
          dx, dw, db = Conv_ReLU_Pool.backward(dscores, cache_dict[cache_i])
          dscores = dx
          grads[W_i] = dw
          grads[b_i] = db
        else:
          dx, dw, db = Conv_ReLU.backward(dscores, cache_dict[cache_i])
          dscores = dx
          grads[W_i] = dw
          grads[b_i] = db
    ...
    for i in range(1, self.num_layers + 1):
      temp_key = 'W' + str(i)
      loss += self.reg * torch.sum(torch.square(self.params[temp_key]))
      grads[temp_key] += 2 * self.reg * self.params[temp_key]
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims=input_dims, num_classes=10,
                      ...
                      weight_scale='kaiming',
                      batchnorm=False,
                      reg=1e-3,  dtype=dtype, device=device)
  ...
  solver = Solver(model, data_dict, update_rule=adam, 
                  ...
                  lr_decay=0.9, num_epochs=25, batch_size=128, 
                  print_every=200, device=device)
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      x_norm = (x - sample_mean.T) / torch.sqrt(sample_var.T + eps)
  ...
  def backward(dout, cache):
    ...
    # using chain rule formulas from paper
    m = dout.shape[0]
    dx_norm = dout * cache['gamma']

    if cache['mode'] == 'train':
      dsample_var = torch.sum(dx_norm * (cache['x']- cache['sample_mean']) * (-1/2) * 
                              (cache['sample_var'] + cache['eps'])**(-3/2), dim=0)

      dsample_mean = (torch.sum(dx_norm * (-1 / torch.sqrt(cache['sample_var'] + \
                      cache['eps'])), dim=0)) + (dsample_var * (1 / m) * \
                      torch.sum(-2 * (cache['x'] - cache['sample_mean']), dim=0))

      dx = (dx_norm * (1 / torch.sqrt(cache['sample_var'] + cache['eps']))) + \
            (dsample_var * (2 * (cache['x'] - cache['sample_mean']) / m)) + \
            (dsample_mean / m)

    else: #test mode- drunning_mean, drunning_var = 0
      dx = dx_norm * (1 / torch.sqrt(cache['running_var'] + cache['eps']))
  ...
  def backward_alt(dout, cache):
    ...
    K = var + eps
    x_diff = x - mean
    dx = (1 / N) * gamma * K**(-0.5) * (N * dout - torch.sum(dout, dim=0) - \
          x_diff * K**(-1.0) * torch.sum(dout * x_diff, dim=0))
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    # x dims: (N, C, H, W) -> (N * H * W, C)
    x = x.permute(0, 2, 3, 1)
    x = x.reshape(N * H * W, C)

    temp, cache = BatchNorm.forward(x, gamma, beta, bn_param)

    # out dims: (N * H * W, C) -> (N, C, H, W)
    out = temp.reshape(N, H, W, C)
  ...
  def backward(dout, cache):
    ...
    # dout dims: (N, C, H, W) -> (N * H * W, C)
    dout = dout.permute(0, 2, 3, 1)
    dout = dout.reshape(N * H * W, C)

    dx_temp, dgamma, dbeta = BatchNorm.backward(dout, cache)

    # dx dims: (N * H * W, C) -> (N, C, H, W)
    dx = dx_temp.reshape(N, H, W, C) 
