...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      # dists between all train-test imgs | train img i to test img j | L2 dists
      test_data = x_test[j, :].view(-1)
      train_data = x_train[i, :].view(-1)

      dists[i, j] = torch.sum(torch.square(train_data - test_data))
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_test):
    test_data = x_test[i, :].view(-1)
    train_data = x_train[:].view(x_train.shape[0], -1)
    
    dists[:, i] = torch.sum(torch.square(test_data - train_data), dim = 1)
...
def compute_distances_no_loops(x_train, x_test):
  ...
  test_data = x_test.view(x_test.shape[0], -1)
  train_data = x_train.view(x_train.shape[0], -1)
 
  test_sum = torch.sum(torch.square(test_data), dim = 1, keepdim=True)
  train_sum = torch.sum(torch.square(train_data), dim = 1, keepdim=True)
  data_dot = torch.matmul(train_data, test_data.T)
  
  # Euclidean dist formula: sqrt(xi^2 + yi^2 - 2*xi*yi)
  dists = test_sum.T + train_sum - 2 * data_dot
...
def predict_labels(dists, y_train, k=1):
  ...
  for j in range(num_test): 
    _, closest_ind = torch.topk(dists[:, j], k, largest=False)
    closest_y = y_train[closest_ind]

    # Pick the most common label (Tie-breaker: Smaller label)
    mode_val, _ = torch.mode(closest_y)
    y_pred[j] = mode_val
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    #initialize accuracies list for key 'k'
    k_to_accuracies[k] = [] 
    for i in folds:
      # Generate train and test folds
      x_train_fold = torch.cat([x_train_folds[j] for j in folds if i is not j])
      y_train_fold = torch.cat([y_train_folds[j] for j in folds if i is not j])
      x_test_fold = x_train_folds[i]
      y_test_fold = y_train_folds[i]

      # Perform knn
      classifier = KnnClassifier(x_train_fold, y_train_fold)
      accuracy = classifier.check_accuracy(x_test_fold, y_test_fold, k)
...
def knn_get_best_k(k_to_accuracies):
  ...
  for key, val in k_to_accuracies.items():
    # Find mean of accuracies for each k
    avg_acc[key] = statistics.mean(val)
