...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        mod_input = int(np.sqrt(self.input_size))
        self.decoder = nn.Sequential(nn.Linear(self.latent_size, self.hidden_dim),
                                     ...
                                     nn.Sigmoid(),
                                     nn.Unflatten(1, (1, mod_input, mod_input)))
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        mod_input = int(np.sqrt(self.input_size))
        self.decoder = nn.Sequential(nn.Linear(self.latent_size + self.num_classes, 
                                               self.hidden_dim),
                                     nn.ReLU(),
                                     nn.Linear(self.hidden_dim, self.hidden_dim),
                                     nn.ReLU(),
    ...
    def forward(self, x, c):
        ...
        flat = nn.Flatten()
        enc_in = self.encoder(torch.cat((flat(x), c), dim=1))
        mu = self.mu_layer(enc_in)
        logvar = self.logvar_layer(enc_in)
        z = reparametrize(mu, logvar)
...
def reparametrize(mu, logvar):
    ...
    N, Z = mu.shape
    # z = sigma * eps + mu
    z = torch.sqrt(torch.exp(logvar)) * torch.randn((N, Z), 
                                                    dtype=mu.dtype, 
                                                    device=mu.device, 
...
def loss_function(x_hat, x, mu, logvar):
    ...
    B, _, H, W = x.shape
    rec = F.binary_cross_entropy(x_hat, x, reduction='sum') * 1.0 / B
    div = -0.5 * torch.sum(1 + logvar - mu**2 - torch.exp(logvar)) / B
    loss = rec + div
