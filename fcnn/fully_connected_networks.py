...
class Linear(object):
  ...
  def backward(dout, cache):
    ...
    dw = torch.reshape(x, (x.shape[0], -1))
    dw = torch.matmul(dw.T, dout)
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    out, cache1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
    scores, cache2 = Linear.forward(out, self.params['W2'], self.params['b2'])
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(self.num_layers):
      W_key = 'W' + str(i + 1)
      b_key = 'b' + str(i + 1)
      self.params[b_key] = torch.zeros(dim[i + 1], dtype=dtype, device=device)
      self.params[W_key] = weight_scale *  torch.randn(dim[i], dim[i + 1], 
  ...
  def loss(self, X, y=None):
    ...
    # arch: {linear - relu - [dropout]} x (L - 1) - linear - softmax
    scores = X
    ...
    for i in range(1, self.num_layers + 1):
      ...
      b_i = 'b' + str(i)
      cache_i = 'c' + str(i)
      drop_i = 'dropout' + str(i)

      if i == self.num_layers:
        scores, cache = Linear.forward(scores, self.params[W_i], 
                                       self.params[b_i])
      else:
        scores, cache = Linear_ReLU.forward(scores, self.params[W_i], 
                                            self.params[b_i])
        if self.use_dropout:
          scores, drop_cache_dict[drop_i] = Dropout.forward(scores, 
                                                            self.dropout_param)
    ...
    for i in range(self.num_layers):
            loss += 0.5 * self.reg * torch.sum(torch.square(self.params['W' + str(i + 1)]))
    ...
    for i in range(self.num_layers, 0, -1):
      ...
      if i == self.num_layers:
        dscores, grads[W_i], grads[b_i] = Linear.backward(dscores, 
                                                          cache_dict[cache_i])
      else:
        if self.use_dropout:
          dscores = Dropout.backward(dscores, drop_cache_dict[drop_i])

        dscores, grads[W_i], grads[b_i] = Linear_ReLU.backward(dscores, 
                                                               cache_dict[cache_i])
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = Solver(model, data_dict, update_rule=sgd, 
                  optim_config={'learning_rate': 1e-1,},
                  lr_decay=0.95, num_epochs=15, batch_size=128, 
...
def sgd_momentum(w, dw, config=None):
  ...
  v = (v * config.get('momentum')) - (dw * config.get('learning_rate'))
  next_w = w + v
...
def rmsprop(w, dw, config=None):
  ...
  next_w = w - (learning_rate / (torch.sqrt(cache) + epsilon)) * dw
...
def adam(w, dw, config=None):
  ...
  next_w = w - (learning_rate * m_t / (torch.sqrt(v_t) + epsilon))
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      # mask = mask.type(x.dtype) / (1 - p)
      # out = x * mask

      mask = torch.rand(*x.shape, dtype=x.dtype, device=x.device)
      mask[mask < p] = 0
      mask[mask > p] = 1
      mask = mask.type(x.dtype) / (1 - p)
      out = x * mask
