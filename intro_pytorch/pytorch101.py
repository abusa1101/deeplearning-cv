...
def count_tensor_elements(x):
  ...
  shape_tensor = torch.tensor(x.shape)
  num_elements = 1
...
def multiples_of_ten(start, stop):
  ...
  x = [i for i in torch.arange(start, stop + 1) if i % 10 == 0]
...
def reshape_practice(x):
  ...
  y = x.contiguous()
  y = y.view(2, 3, 4).transpose(1, 2).reshape(8, 3).t()
...
def normalize_columns(x):
  ...
  y = x.clone()
  mu_j = torch.sum(y, dim = 0) / x.shape[0]
  sigma_j = torch.sqrt(torch.sum(torch.square(y - mu_j), dim = 0) / (x.shape[0] - 1))
